...
# --- 2. CSS: DARK MODE & UKRYWANIE "SŁOŃCA" ---
st.markdown("""
    <style>
    /* Ukrycie paska narzędzi Streamlit (hamburgera, słońca/ustawień) */
    [data-testid="stToolbar"] {visibility: hidden !important;}
    header {visibility: hidden !important;}
    footer {visibility: hidden !important;}

    /* Wymuszenie ciemnego tła i kolorów (na wypadek ustawień przeglądarki) */
    .stApp {
        background-color: #1e1e1e;
        color: #e0e0e0;
    }

    /* Stylizacja Przycisków - Twój Fiolet */
    div.stButton > button {
        width: 100%;
    ...
    /* Stylizacja Inputów i Selectboxów na ciemno */
    ...
    /* Wyśrodkowanie obrazka w sidebarze */
...
LIGI_KODY = {
    ...
    "Brazylia - Serie A": "BRA", "Argentyna - Liga Pro": "ARG", "Chiny - Super League": "CHN",
    "Japonia - J-League": "JPN"
}
...
def pobierz_dane(ile_lat):
    curr_y = 25;
    start_y = curr_y - ile_lat
    sezony = [f"{i:02d}{i + 1:02d}" for i in range(start_y, curr_y + 1)];
    sezony.reverse()
    wszystkie = []

    # Placeholder na status (zamiast print)
    status = st.empty()
    ...
    for s in sezony:
        for n, k in LIGI_KODY.items():
            ...
            if done % 50 == 0: status.text(f"Pobieranie historyczne... {int(done / total * 100)}%")

    status.text("Pobieranie bieżących sezonów...")
    for n, k in LIGI_KODY.items():
        try:
            r = requests.get(f"https://www.football-data.co.uk/new/{k}.csv", timeout=1)
            if r.status_code == 200:
                df = pd.read_csv(io.StringIO(r.text));
                df = clean_df(df, n, "Current")
                wszystkie.append(df)
        except:
            pass
...
tabs = st.tabs([
    ...
    "Dokładny Wynik", "Rożne", "Kartki", "BTTS", "Perełki", "Słownik"
])
...
with tabs[0]:
    ...
    if st.button("Skanuj Bazę"):
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.subheader("Top Drużyny")
            if res: st.dataframe(pd.DataFrame(res).sort_values('Łamaki', ascending=False).head(20),
...
# 2. PRZECIWNIK
with tabs[1]:
    st.subheader("Szukaj Ofiary")
    mt = st.text_input("Twoja drużyna (Gospodarz):")
    if st.button("Szukaj") and mt:
        ...
        if rh:
            ...
            if cand:
                st.dataframe(pd.DataFrame(cand).sort_values('Wpadki', ascending=False).head(20),
                             use_container_width=True)
            else:
                st.info("Brak.")
...
# 3. ŁAMAK (Fixed Logic)
with tabs[2]:
    st.subheader("Analiza Łamaka 1/2")
    c1, c2 = st.columns(2)
    h = c1.text_input("Gosp:", key="t3h");
    a = c2.text_input("Gość:", key="t3a")
    if st.button("Analizuj"):
        rh, ra = find_teams(df, h, a)
        if rh and ra:
            # A. HISTORIA
            h_all = df[(df['HomeTeam'] == rh) | (df['AwayTeam'] == rh)]
            h_lam = len(h_all[((h_all['HTR'] == 'H') & (h_all['FTR'] == 'A')) | (
                        (h_all['HTR'] == 'A') & (h_all['FTR'] == 'H'))])
            a_all = df[(df['HomeTeam'] == ra) | (df['AwayTeam'] == ra)]
            a_lam = len(a_all[((a_all['HTR'] == 'H') & (a_all['FTR'] == 'A')) | (
                        (a_all['HTR'] == 'A') & (a_all['FTR'] == 'H'))])

            # B. SCENARIUSZ
            mh = df[df['HomeTeam'] == rh];
            ma = df[df['AwayTeam'] == ra]
...
with tabs[3]:
    st.subheader("Serie H2H w miesiącu")
    m = st.slider("Miesiąc:", 1, 12, datetime.now().month)
    if st.button("Szukaj Serii"):
        # Szukamy PAR, które grały w tym miesiącu w przeszłości i wystąpił łamak
        lam = df[(((df['HTR'] == 'H') & (df['FTR'] == 'A')) | ((df['HTR'] == 'A') & (df['FTR'] == 'H'))) & (
                    df['Miesiac'] == m)]
        if not lam.empty:
            ...
            for (h_t, a_t), g in pairs:
                ...
                # Pokazujemy nawet pojedyncze przypadki, żeby użytkownik wiedział, że coś było
                lata_str = " -> ".join([str(int(y)) for y in yrs])
                res.append({'Mecz': f"{h_t} vs {a_t}", 'Lata z Łamakiem': lata_str})
            ...
            st.info("Brak łamaków w tym miesiącu.")

# 5. GOLE
with tabs[4]:
    st.subheader("Gole Over/Under")
    c1, c2 = st.columns(2)
    h5 = c1.text_input("H:", key="t5h");
    a5 = c2.text_input("A:", key="t5a")
    if st.button("Oblicz"):
...
with tabs[5]:
    st.subheader("Remis")
    c1, c2 = st.columns(2);
    h6 = c1.text_input("H:", key="t6h");
    a6 = c2.text_input("A:", key="t6a")
    if st.button("Sprawdź"):
        rh, ra = find_teams(df, h6, a6)
        if rh:
            p1 = len(df[(df['HomeTeam'] == rh) & (df['FTR'] == 'D')]) / len(df[df['HomeTeam'] == rh]) * 100 if len(
                df[df['HomeTeam'] == rh]) else 0
            p2 = len(df[(df['AwayTeam'] == ra) & (df['FTR'] == 'D')]) / len(df[df['AwayTeam'] == ra]) * 100 if len(
                df[df['AwayTeam'] == ra]) else 0
            st.metric("Szansa", f"{(p1 + p2) / 2:.1f}%")

# 7. DOKŁADNY WYNIK
with tabs[6]:
    st.subheader("Symulacja Wyniku")
    c1, c2 = st.columns(2);
    h7 = c1.text_input("H:", key="t7h");
    a7 = c2.text_input("A:", key="t7a")
    if st.button("SYMULUJ"):
        rh, ra = find_teams(df, h7, a7)
        if rh:
            try:
                l = df[df['HomeTeam'] == rh]['Liga'].iloc[0];
                dfl = df[df['Liga'] == l].tail(500)
                avg_hg = dfl['FTHG'].mean();
                avg_ag = dfl['FTAG'].mean()
                ...
                for i in range(5):
                    for j in range(5):
                        p = (math.exp(-xg_h) * (xg_h ** i) / math.factorial(i)) * (
                                    math.exp(-xg_a) * (xg_a ** j) / math.factorial(j)) * 100
...
with tabs[7]:
    st.subheader("Rożne")
    c1, c2 = st.columns(2);
    h8 = c1.text_input("H:", key="t8h");
    a8 = c2.text_input("A:", key="t8a")
    if st.button("Analiza Rożnych"):
        rh, ra = find_teams(df, h8, a8)
        if rh:
            dfc = df[df['HC'] > 0]
            mh = dfc[dfc['HomeTeam'] == rh];
            ma = dfc[dfc['AwayTeam'] == ra]
            if not mh.empty and not ma.empty:
                havg = mh['HC'].mean() + mh['AC'].mean();
                aavg = ma['HC'].mean() + ma['AC'].mean()
...
with tabs[8]:
    st.subheader("Kartki")
    c1, c2 = st.columns(2);
    h9 = c1.text_input("H:", key="t9h");
    a9 = c2.text_input("A:", key="t9a")
    if st.button("Analiza Kartek"):
        ...
        if rh:
            ...
            ma = dfc[dfc['AwayTeam'] == ra]
            if not mh.empty and not ma.empty:
                hpts = (mh['HY'] + 2 * mh['HR']).mean();
                apts = (ma['AY'] + 2 * ma['AR']).mean()
...
with tabs[9]:
    st.subheader("BTTS")
    c1, c2 = st.columns(2);
    h10 = c1.text_input("H:", key="t10h");
    a10 = c2.text_input("A:", key="t10a")
    if st.button("Sprawdź BTTS"):
        rh, ra = find_teams(df, h10, a10)
        if rh:
            mh = df[df['HomeTeam'] == rh];
            hp = len(mh[(mh['FTHG'] > 0) & (mh['FTAG'] > 0)]) / len(mh) * 100 if len(mh) else 0
            ma = df[df['AwayTeam'] == ra];
            ap = len(ma[(ma['FTHG'] > 0) & (ma['FTAG'] > 0)]) / len(ma) * 100 if len(ma) else 0
            st.metric("Szansa", f"{(hp + ap) / 2:.1f}%")

# 11. PEREŁKI (Bez koszyków)
with tabs[10]:
    st.subheader("Generator Perełek")
    ...
    if st.button("Generuj Listy"):
        with st.spinner("Przetwarzanie..."):
            ...
            for t in all_teams:
                ...
                h_leads = len(mh[mh['HTR'] == 'H']);
                h_chokes = len(mh[(mh['HTR'] == 'H') & (mh['FTR'] == 'A')])
                h_risk = (h_chokes / h_leads * 100) if h_leads > 0 else 0
                a_trails = len(ma[ma['HTR'] == 'H']);
                a_come = len(ma[(ma['HTR'] == 'H') & (ma['FTR'] == 'A')])
        ...
        with c1:
            ...
            if real:
                st.dataframe(pd.DataFrame(real).sort_values('prob_num', ascending=False).drop(columns=['prob_num']),
                             use_container_width=True)
            else:
                st.info("Brak.")
        with c2:
            ...
            if glo:
                st.dataframe(
                    pd.DataFrame(glo).sort_values('prob_num', ascending=False).head(200).drop(columns=['prob_num']),
                    use_container_width=True)
            else:
                st.info("Brak.")
