...
# --- 2. CSS (STABILNY DLA SAFARI & MOBILE) ---
st.markdown("""
    <style>
    /* 1. Header widoczny ale przezroczysty (kluczowe dla Safari) */
    header {visibility: visible !important; background: transparent !important;}
    
    /* 2. Ukrycie elementów dekoracyjnych Streamlit */
    [data-testid="stDecoration"] {display: none !important;}
    [data-testid="stToolbar"] {visibility: hidden !important; height: 0px !important;}
    footer {visibility: hidden !important;}
    
    /* 3. WYMUSZENIE STRZAŁKI (Obsługa iPhone) */
    ...
    div.stButton > button:hover {
        ...
        box-shadow: 0 6px 20px rgba(135, 66, 245, 0.5);
    }
    
    /* Inputy */
    [data-testid="stTextInput"] input, [data-testid="stSelectbox"] > div > div {
        background-color: #2d2d2d !important; color: white !important; border: 1px solid #444; border-radius: 8px;
    }
...
# --- FUNKCJE POMOCNICZE (DATA CLEANING) ---
def clean_df(df, n, s):
...
# --- SILNIK ANALITYCZNY (PROFESSIONAL ENGINE) ---
# To jest serce obliczeń "League Relative Strength"
def calculate_metrics(df, rh, ra):
    try:
        # 1. Kontekst ligowy (ostatnie 500 meczów w lidze dla średniej)
        ...
        # 2. Statystyki drużyn (Forma - ostatnie 20 meczów)
        mh = df[df['HomeTeam'] == rh].tail(20)
        ma = df[df['AwayTeam'] == ra].tail(20)
        
        if mh.empty or ma.empty: return None
        
        # 3. Siła Ataku i Obrony
        # Atak Dom = (Gole strzelone w domu / mecze) / Średnia goli dom w lidze
...
if 'df' not in st.session_state or st.session_state['df'].empty:
    st.info("👈 Rozwiń menu (strzałka) i pobierz bazę danych.")
    st.stop()

df = st.session_state['df']

# ==============================================================================
# LOGIKA ZAKŁADEK (PROFESSIONAL REWORK)
# ==============================================================================
...
if selected_page == "1. Schematy":
    if st.button("Skanuj Trendy"):
        ...
        for t in teams:
            ...
            if len(d)<5: continue
            # Szukamy serii bez remisu (przykład trendu)
            last_10 = d.tail(10)
            no_draws = len(last_10[last_10['FTR']!='D'])
            # Łamaki
            hard = len(d[((d['HTR']=='H')&(d['FTR']=='A'))|((d['HTR']=='A')&(d['FTR']=='H'))])
            if hard>0: res.append({'Drużyna':t, 'Mecze':len(d), 'Łamaki':hard, 'Bez Remisu (ost. 10)': no_draws})
        ...
        with c2:
            st.subheader("🔥 Serie H2H (Dom/Wyjazd)")
    ...
    if st.button("Szukaj Ofiary") and mt:
        ...
        if rh:
            ...
            for op in (set(df['HomeTeam'])|set(df['AwayTeam'])):
                if op==rh: continue
                # Szukamy drużyn które często tracą prowadzenie
                ma=df[df['AwayTeam']==op]
                choke=len(ma[(ma['HTR']=='A')&(ma['FTR']=='H')])
                # Albo tracą dużo goli na wyjeździe
                goals_conc = ma['FTHG'].mean() if not ma.empty else 0
                if choke>0 or goals_conc > 1.5: 
                    cand.append({'Rywal':op, 'Wpadki (Łamak)':choke, 'Śr. stracone gole': f"{goals_conc:.2f}"})
            if cand: st.dataframe(pd.DataFrame(cand).sort_values('Wpadki (Łamak)',ascending=False).head(20), use_container_width=True)
            else: st.info("Brak idealnych kandydatów.")
    ...
    if st.button("Analiza HT/FT"):
        ...
        if rh and ra:
            ...
            # Ile razy gosp oddał prowadzenie
            h_lead = len(mh[mh['HTR']=='H'])
            h_choke = len(mh[(mh['HTR']=='H')&(mh['FTR']=='A')])
            h_pct = (h_choke/h_lead*100) if h_lead > 0 else 0
            
            # Ile razy gość odrobił
            a_trail = len(ma[ma['HTR']=='H'])
            a_come = len(ma[(ma['HTR']=='H')&(ma['FTR']=='A')])
            a_pct = (a_come/a_trail*100) if a_trail > 0 else 0
            
            avg_prob = (h_pct + a_pct) / 2
            
            st.metric("Szansa Matematyczna (HT/FT 1/2)", f"{avg_prob:.1f}%")
            
            c_res1, c_res2 = st.columns(2)
            with c_res1:
                st.info(f"**{rh} (Dom)**")
                st.write(f"Prowadził do przerwy: {h_lead}")
                st.write(f"Przegrał mecz (Choke): {h_choke}")
                st.write(f"Podatność: {h_pct:.1f}%")
            with c_res2:
                st.error(f"**{ra} (Wyjazd)**")
                st.write(f"Przegrywał do przerwy: {a_trail}")
                st.write(f"Wygrał mecz (Comeback): {a_come}")
                st.write(f"Potencjał: {a_pct:.1f}%")

elif selected_page == "4. H2H Kalendarz":
    m = st.slider("Miesiąc:", 1, 12, datetime.now().month)
    if st.button("Szukaj Historii"):
        # Szukamy łamaków w tym miesiącu w historii
        ...
        if not lam.empty:
            ...
            df_res = pd.DataFrame(res).sort_values('Ilość', ascending=False)
            st.dataframe(df_res, use_container_width=True)
        else: st.info("Brak historycznych łamaków w tym miesiącu.")

# --- ZAAWANSOWANE GOLE (xG + POISSON) ---
    ...
    if st.button("Oblicz Potencjał"):
        ...
        if rh and ra:
            ...
            if metrics:
                ...
                st.markdown("### 📊 Analiza xG (Expected Goals)")
                ...
                st.progress(probs['O2.5'])
            else: st.error("Za mało danych dla tych drużyn.")

# --- ZAAWANSOWANE REMISY (SIMILARITY INDEX) ---
    ...
    if st.button("Analiza Sił"):
        ...
        if rh and ra:
            ...
            if metrics:
                ...
                if diff < 0.2: 
                    st.success("🔥 BARDZO WYSOKIE RYZYKO REMISU! (Siły wyrównane)")
                elif diff < 0.5:
                    st.warning("Możliwy remis.")
                else:
                    st.info("Mała szansa na remis (Wyraźny faworyt).")
                
                st.write(f"Różnica xG: {diff:.2f}")

# --- DOKŁADNY WYNIK (POISSON MATRIX) ---
    ...
    if st.button("Symulacja"):
        ...
        if rh and ra:
            ...
            if metrics:
                ...
                # Ładna tabela
                df_res = pd.DataFrame(matrix, columns=['Wynik', 'Szansa %']).head(5)
                st.table(df_res)
            else: st.error("Brak danych.")

# --- ROŻNE (CORNER PRESSURE MODEL) ---
    ...
    if st.button("Analiza Rożnych"):
        ...
        if rh:
            # Filtrujemy tylko mecze z danymi o rożnych
            dfc = df[df['HC']>0]
            mh = dfc[dfc['HomeTeam']==rh].tail(10)
            ma = dfc[dfc['AwayTeam']==ra].tail(10)
            
            if not mh.empty and not ma.empty:
                # Model: Ile A nabija + Ile B pozwala / 2
                h_corners = (mh['HC'].mean() + ma['HC'].mean()) / 2
                a_corners = (ma['AC'].mean() + mh['AC'].mean()) / 2
                total = h_corners + a_corners
                
                st.metric("Linia Rożnych", f"{total:.1f}")
                
                c_corn1, c_corn2 = st.columns(2)
                with c_corn1:
                    st.write(f"**{rh}:** ~{h_corners:.1f}")
                with c_corn2:
                    st.write(f"**{ra}:** ~{a_corners:.1f}")
            else: st.warning("Brak danych o rożnych dla tej ligi.")

# --- KARTKI (BOOKING POINTS) ---
    ...
    if st.button("Analiza Agresji"):
        ...
        if rh:
            ...
            mh = dfc[dfc['HomeTeam']==rh].tail(15)
            ma = dfc[dfc['AwayTeam']==ra].tail(15)
            
            if not mh.empty and not ma.empty:
                # Booking points: Y=10, R=25
                h_pts = (mh['HY']*10 + mh['HR']*25).mean()
                a_pts = (ma['AY']*10 + ma['AR']*25).mean()
                
                # Provocation index (ile rywale dostają kartek)
                h_prov = (mh['AY']*10 + mh['AR']*25).mean()
                a_prov = (ma['HY']*10 + ma['HR']*25).mean()
                
                exp_pts = ((h_pts + a_prov)/2) + ((a_pts + h_prov)/2)
                
                st.metric("Punkty Kartkowe (Linia)", f"{exp_pts:.0f}")
                st.caption("Żółta = 10 pkt, Czerwona = 25 pkt")
                
                if exp_pts > 50: st.error("⚔️ BARDZO OSTRY MECZ!")
                elif exp_pts < 30: st.success("Mecz przyjaźni.")
                else: st.warning("Standardowa walka.")
    ...
    if st.button("Sprawdź BTTS"):
        ...
        if rh and ra:
            ...
            if metrics:
                ...
                if btts_prob > 60: st.success("Wysoka szansa na gole z obu stron!")
                else: st.error("Jedna z drużyn może nie strzelić.")
...
# --- PEREŁKI (VALUE BETS) ---
    ...
    if st.button("Szukaj Okazji"):
        with st.spinner("Analizuję całą bazę..."):
            # Analiza uproszczona dla szybkości, ale oparta na xG
            matches = []
            
            # Pobieramy unikalne pary z ostatnich meczów (żeby nie liczyć tysięcy razy)
            recent_games = df.tail(200) # Ostatnie 200 meczów w bazie
            ...
            for index, row in recent_games.iterrows():
                ...
                if metrics:
                    ...
                    # Szukamy Home Win > 70% lub Over 2.5 > 65%
                    p_home = probs['1'] * 100
                    p_over = probs['O2.5'] * 100
                    
                    if p_home >= thr:
                        matches.append({'Mecz': f"{h} vs {a}", 'Typ': '1 (Dom)', 'Szansa': f"{p_home:.1f}%", 'xG': f"{xg_h:.2f}-{xg_a:.2f}"})
                    elif p_over >= thr:
                        matches.append({'Mecz': f"{h} vs {a}", 'Typ': 'Over 2.5', 'Szansa': f"{p_over:.1f}%", 'xG': f"{xg_h:.2f}-{xg_a:.2f}"})
            
            if matches:
                st.success(f"Znaleziono {len(matches)} okazji w ostatnich danych!")
                st.dataframe(pd.DataFrame(matches), use_container_width=True)
            else:
                st.info("Brak wyraźnych okazji przy tych parametrach.")
